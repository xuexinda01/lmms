...
from llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, DEFAULT_VIDEO_TOKEN
...
class ModelArguments:
    ...
    add_faster_video: Optional[bool] = field(default=False)   #TODO 是否减少video tokens的数量
    faster_token_stride: Optional[int] = field(default=10)   #TODO  default_stride=2
...
class DataArguments:
    ...
    image_folder: Union[str, List[str]] = field(default=None)
    ...
    force_sample: Optional[bool] = field(default=False)   #TODO  此处强制采样 frames_upbound 数量的 frames
    multi_img_num: Optional[int] = field(default=16)   #强制load多图数据等间隔采样, 最多支持multi_img_num个图片，防止load数据OOM
    downsample_images: Optional[bool] = field(default=False)  #TODO True ==> 对image tokens 从729 下采样到196 tokens 
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if trainer.deepspeed:  
        ...
        # del state_dict
        ## DEBUG by Henry Zhang
        # for key in cpu_state_dict.keys():
        #     if cpu_state_dict[key].shape == torch.Size([152064, 3584]):
        #         cpu_state_dict[key] = cpu_state_dict[key][:151936]
...
#     return ''.join(result)
def sample_and_replace(text, n, img_token):
    """等间隔采样img,避免load sample中img过多OOM. from henry
    """
    # find img position
    matches = [match.start() for match in re.finditer(img_token, text)]
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        for sentence in source:
            ...
            sentence["value"] = sentence["value"].replace(DEFAULT_VIDEO_TOKEN, DEFAULT_IMAGE_TOKEN).strip()  #<video> token  => <image> token
            
            num_im = len(re.findall(DEFAULT_IMAGE_TOKEN, sentence["value"]))
            if num_im > multi_img_num:
                # sentence = sample_and_replace(sentence, multi_img_num, DEFAULT_IMAGE_TOKEN)  #by yuheng
                sentence["value"] = sample_and_replace(sentence["value"], multi_img_num, DEFAULT_IMAGE_TOKEN)
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    # 151665
    # print(tokenizer)
    if '2.5' in tokenizer.name_or_path:
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        #Here only support for one image folder
        # try:
        ...
        if isinstance(image_folder, str):
            image_folder = [image_folder]  # 将单个目录转换为列表以简化处理

        image = None
        last_exception = None  # 用于存储最后一个异常

        for folder in image_folder:
            try:
                image = Image.open(os.path.join(folder, image_file)).convert("RGB")
                break  # 成功打开图像文件后，跳出循环
            except Exception as exn:
                last_exception = exn  # 记录最后一个异常，但不立即输出

        # 如果没有成功打开图像，则抛出最后捕获的异常
        if image is None:
            print(f"Failed to open image {image_file} in all provided directories.")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                #by henry  for solving OOM
                
                # if len(image_file) > 1:
                #     image = [self.process_image(f, "pad") for f in image_file]
                #     image = [[im[0], im[1], "image"] for im in image]
                if len(image_file) > self.data_args.multi_img_num: # 等间隔采样，多图直接pad，不执行anyres
                    ...
                    image = [self.process_image(f, "pad") for f in image_file] # 多图pad
                    image = [[im[0], im[1], "image"] for im in image]
                else:
                    image = [self.process_image(f) for f in image_file] # 单图anyres
                    image = [[im[0], im[1], "image"] for im in image]
            else:
...
def train(attn_implementation=None):
    ...
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
        ...
        trainer.train()
    trainer.save_state()  # TODO  here  save the trainer_state.json
    ...
    # #test => trainer._save(output_dir, state_dict=cpu_state_dict)  
    # trainer.save_state()
    # safe_save_model_for_hf_trainer(trainer=trainer, output_dir=training_args.output_dir)
